	...
	assignExpressionAtPath ( path, expression ) {
		...
		if ( path.length > 0 ) {
			this.assignedExpressions.forEachAtPath( path.slice( 0, -1 ), ( relativePath, node ) =>
				node.bindAssignmentAtPath( relativePath.concat( path.slice( -1 ) ), expression ) );
		}
		if ( path.length === 0 ) {
...
class MemberExpression extends Node$1 {
	...
	bindAssignmentAtPath ( path, expression ) {
		...
		if ( this.variable ) {
			...
			this.object.bindAssignmentAtPath( [ UNKNOWN_KEY ].concat( path ), expression );
		} else {
			this.object.bindAssignmentAtPath( [ this.property.name ].concat( path ), expression );
		}
	}
	...
	hasEffectsWhenAccessedAtPath ( path, options ) {
		...
		return this.object.hasEffectsWhenAccessedAtPath( [ this.computed ? UNKNOWN_KEY : this.property.name ].concat( path ), options );
	}
	...
	hasEffectsWhenAssignedAtPath ( path, options ) {
		...
		return this.object.hasEffectsWhenAssignedAtPath( [ this.computed ? UNKNOWN_KEY : this.property.name ].concat( path ), options );
	}
	...
	hasEffectsWhenCalledAtPath ( path, options ) {
		...
		return this.computed
			|| this.object.hasEffectsWhenCalledAtPath( [ this.property.name ].concat( path ), options );
	}
	...
	hasEffectsWhenMutatedAtPath ( path, options ) {
		...
		return this.object.hasEffectsWhenMutatedAtPath( [ this.property.name ].concat( path ), options );
	}
...
class ObjectExpression extends Node$1 {
	...
	hasEffectsWhenAssignedAtPath ( path, options ) {
		...
		var ref = this._getPossiblePropertiesWithName( path[ 0 ], PROPERTY_KINDS_WRITE );
		var properties = ref.properties;
		var hasCertainHit = ref.hasCertainHit;
...
class VariableDeclarator extends Node$1 {
	bindAssignmentAtPath ( path, expression ) {
		this.id.bindAssignmentAtPath( path, expression );
	}
